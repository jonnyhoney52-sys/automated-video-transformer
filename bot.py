...
async def progress_callback(current, total, msg, action):
    if total == 0: return
    if current % (1024 * 1024 * 5) == 0 or current == total:  # Update every 5MB
        try:
...
# --- Hacker AI Supreme Transform Engine ---
async def supreme_transform_video(input_path, output_path, msg):
    await msg.edit_text("🔍 **Analyzing Video & Randomizing Parameters...**")
    ...
    pitch_factor = round(2 ** (rng.uniform(-2.5, -0.5) / 12.0), 6)
    tgt_fps = rng.choice([23.976, 24, 25, 29.97, 30])
    gop_size = rng.randint(12, 250)

    zoom_w, zoom_h = int(W * zoom), int(H * zoom)
    crop_x = (zoom_w - W) // 2 + rng.randint(0, 3)
    crop_y = (zoom_h - H) // 2 + rng.randint(0, 3)

    vf = f"scale={zoom_w}:{zoom_h}:flags=lanczos,crop={W}:{H}:{crop_x}:{crop_y}"
    vf += f",eq=brightness={round(rng.uniform(0.03, 0.08), 3)}:contrast={round(rng.uniform(0.01, 0.05), 3)}"
    vf += f",noise=c0s={rng.randint(6, 15)}:c0f=t+u,setpts={1/spd}*PTS,fps=fps={tgt_fps}:round=up"
    ...
    cmd = [
        ...
        "-x264-params", f"keyint={gop_size}:min-keyint={max(1, gop_size//2)}",
        "-vf", vf
    ]
    
    if info["has_audio"]:
        cmd += ["-c:a", "aac", "-b:a", "128k", "-af", f"rubberband=pitch={pitch_factor}:transients=crisp"]
    else:
        cmd += ["-an"]
